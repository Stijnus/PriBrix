...
- [ ] Implement `app/(tabs)/_layout.tsx`:
...
- [ ] Create `src/features/sets/components/SetCard.tsx`:
  - Reusable set card component
  - NativeWind styled
  - Tap navigates to set detail
- [ ] Handle empty state: "No prices available yet"
...
- [ ] Optional: add filters (theme, year, price range) — can defer to later

## 4. Set Detail Screen

- [ ] Create route `app/set/[setNum].tsx`:
  - Dynamic route by set number
- [ ] Create `src/features/sets/api.ts` (extend):
...
- [ ] Create `src/features/sets/hooks.ts` (extend):
  - `useSetDetail(setNum)` — React Query hook
- [ ] Implement Set Detail screen:
...
- [ ] Create `src/features/sets/components/OfferRow.tsx`:
  - Retailer name + logo placeholder
  - Price display (base + delivered when available)
  - Stock status badge
  - "Buy" / affiliate link button
- [ ] Create `src/features/sets/components/PriceHistoryChart.tsx` (stub):
...
- [ ] Create `src/lib/affiliate/openLink.ts`:
...
- [ ] Create `src/features/wishlist/hooks.ts`:
  - `useLocalWishlist()` — read/write AsyncStorage
  - Returns `{ items, addItem, removeItem, updateItem }`
- [ ] Create `src/features/wishlist/components/WishlistList.tsx`:
...
- [ ] Create `src/components/ui/Badge.tsx` (stock status, priority, etc.)
- [ ] Create `src/components/ui/PriceDisplay.tsx` (formatted EUR price)

---

## Verification

- [ ] Home screen loads and shows set cards with prices
- [ ] Search finds sets by name and set number
- [ ] Set Detail shows offers with prices and retailer info
- [ ] "Buy" button opens affiliate link in in-app browser
- [ ] Can add/remove sets to all 3 local lists
- [ ] My LEGO screen shows all 3 tabs with correct data
- [ ] Settings country toggle persists across app restarts
- [ ] App handles missing data gracefully (no price, no shipping, no image)
- [ ] All screens styled with NativeWind (no raw StyleSheet usage)
