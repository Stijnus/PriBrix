...
**Status: COMPLETE** ✓ — Verified 2026-03-07. All 16 implementation files confirmed present and fully implemented (not stubs).
...
- [x] Configure Supabase dashboard:
  - Site URL: `pribrix://`
  - Redirect URL: `pribrix://auth/verify`
- [x] Test deep link opens app (on device/emulator)
...
- [x] Create `src/features/auth/api.ts`:
  ...
  - `completeMagicLinkSignIn()` — handles deep link token exchange
- [x] Create `src/features/auth/hooks.ts`:
  - `useAuth()` — returns `{ user, session, signIn, signOut, isLoading, isMigrating }`
  - Derives from `useSession()` context

## 3. Session Provider Updates

- [x] Update `src/lib/auth/session.tsx`:
...
- [x] Create `src/features/watchlist/api.ts`:
...
- [x] Create `src/features/watchlist/hooks.ts` (update):
...
- [x] Create `src/features/wishlist/api.ts`:
...
- [x] Create `src/features/wishlist/hooks.ts` (update):
  - `useWishlist()` — local OR server based on auth
- [x] Create `src/features/owned/api.ts`:
...
- [x] Create `src/features/owned/hooks.ts` (update):
  - `useOwned()` — local OR server based on auth

## 5. Local-to-Server Migration

- [x] Create `src/lib/migration/migrateLocalLists.ts`:
...
- [x] Wire migration into auth flow:
...
- [x] Add "Sign in" prompt in Settings when not logged in
- [x] Add "Create account to enable alerts" CTA:
...
- [x] Magic link email sends successfully
- [x] Tapping email link opens app and signs user in
- [x] "Resend" button is disabled for 60 seconds after sending
- [x] Expired link shows appropriate error message
- [x] Local lists migrate to server on first login (no duplicates)
- [x] After migration, AsyncStorage local list keys are cleared
- [x] Logged-in user sees server-backed lists
- [x] Logged-out user sees local lists
- [x] Adding/removing items works in both modes
- [x] Sign out switches back to local mode
